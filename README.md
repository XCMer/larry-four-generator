...
If you do not want to extend models generated by Larry Four (which is useful for auto-validating models like [Ardent](https://github.com/laravelbook/ardent) ) you are ready to go. Else, if you are extending something different than Eloquent you can use the published config.
It is available in a subfolder of your app config dir

    /app/config/packages/raahul/larryfour/config.php

Below is an example config file prepared to work with Ardent. 
...
In the example above, all models generated by Larry Four, apart from User, Role and Permission will extend Ardent (whereas the 3 exclusions will extend Eloquent). Additionaly, the models will also have a validation rules array stub placed in. 
In the next revision the ommitModels feature should be rebuilt in such a way, that apart from defining the global and default namespaces and parent class you can define alternative values per model name. 
Furthermore, it'd be nice if some future revision had automatic validation rule creation (based on the fields definitions)
